...
def update_core_geography(*, apps: StateApps, geographies):
    print(f"AIDAN: using {apps} to get model in migration")
    Geography = apps.get_model("data", "Geography")
    print(f"AIDAN have %s existing geographies", Geography.objects.count())
    ...
    for geography in geographies:
        ...
        try:
            ...
            logger.exception(
                "`Geography` %s not found in %s, can't update the associated `geography_code`",
                old,
